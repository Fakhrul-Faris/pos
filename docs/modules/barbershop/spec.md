...
## 2a. Party booking (group)

One **booking record**, one **queue number**, one **bill** — even when multiple people arrive together.

| Rule | Detail |
| ---- | ------ |
| **Party size** | Customer sets `party_size` (1–6) on web; each member picks **their own services** |
| **Queue #** | Single queue number for the whole party (e.g. #42) — not one # per person |
| **Arrival time** | **One shared slot** — all members arrive together |
| **Calendar block** | Slot reserves **sum of member durations** (+ buffers) on the chosen barber’s calendar |
| **Barber preference** | Customer picks **one barber** (or “Anyone”) for the party — **not** per-person barber on web |
| **Chair assignment** | Manager **splits party across barbers** at POS check-in (customer never sees this) |
| **Parallel cuts** | After assign, barbers may cut **in parallel** — one active cut per barber, not sequential |
| **Partial check-in** | If booked 3 but 2 arrive: manager marks Here / No-show per member; **bill excludes no-shows**; slot freed for no-show member |
| **Payment** | Line items per **arrived** member + assigned barber; one checkout, one receipt |
| **Customer status** | Status page shows party progress (who in chair / waiting) — single #42 |

**Data model (conceptual):**

```
booking { party_size, queue_number, slot_start, barber_preference_id }
party_members[] { display_name?, planned_services[], status: expected|arrived|in_chair|done|no_show, assigned_barber_id? }
```

**Penpot / motion reference:** [`../../design/motion-prototype.md`](../../design/motion-prototype.md) · Customer Web + POS party frames.
...
- **Party bookings:** block length = **sum of each member’s** `(duration + buffer)` for the chosen slot start.
...
| State      | Customer web shows (solo)             | Customer web shows (party) |
| ---------- | ------------------------------------- | -------------------------- |
| BOOKED     | “#42 · Ali · Sat 2:00pm · Normal Cut” | “#42 · Party of 3 · Sat 2:00pm” |
| ARRIVED    | “Checked in — please wait”            | “Checked in — please wait” |
| IN_SERVICE | “In chair”                            | “2 of 3 in chair” + per-member hint |
| PAID       | Receipt link                          | Receipt link (all arrived members) |
| NO_SHOW    | “Missed appointment” + rebook CTA     | Whole-party or per-member no-show per §6 |
...
6. **Party booking — partial check-in:** if 2 of 3 arrive, manager toggles **Here / No-show** per member on POS. No-shows are not charged; their calendar portion may be freed. Remaining members keep **one queue #** and proceed to parallel cuts. Whole-party no-show still possible if nobody arrives.
...
| HitPay             | Customer pays **subtotal + 2% service fee**; merchant receives **full subtotal**; cash/own DuitNow = exact subtotal (no fee) |
| HitPay modes       | **QR** (scan + poll until paid) · **Card** (tap on HitPay terminal / phone) — Phase 1B only                                    |
| HitPay example     | Subtotal RM 55.00 → fee RM 1.10 → customer pays **RM 56.10**                                                                 |
| Party + HitPay     | Subtotal = sum of **arrived** members only (e.g. 2 of 3 → RM 80 + 2% = RM 81.60); line items show name · service · barber    |
| Phase 1A           | Cash + Own DuitNow only on POS                                                                                               |
| Phase 1B           | HitPay QR + card enabled per plan tier ([`features-and-pricing.md`](features-and-pricing.md))                                |
...
Party booking when ON: max `party_size` 6; slot block = sum of member durations; POS partial check-in enabled.
